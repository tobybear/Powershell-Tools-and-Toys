...
$NearbyWifi = $wshell = New-Object -ComObject wscript.shell; explorer.exe ms-availablenetworks: ; $wshell.AppActivate("explorer.exe ms-availablenetworks:"); sleep 1; $wshell.SendKeys('{ESC}'); sleep 4; (netsh wlan show networks mode=Bssid | ?{$_ -like "SSID*" -or $_ -like "*Signal*" -or $_ -like "*Band*"}).trim()
