...
$NearbyWifi = $wshell = New-Object -ComObject wscript.shell; explorer.exe ms-availablenetworks: ; sleep 5 ; $wshell.SendKeys('{ESC}'); (netsh wlan show networks mode=Bssid | ?{$_ -like "SSID*" -or $_ -like "*Authentication*" -or $_ -like "*Encryption*" -or $_ -like "*Signal*" -or $_ -like "*Band*"}).trim()
